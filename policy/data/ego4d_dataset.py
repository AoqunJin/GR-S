    ...
    def __init__(self,
        ...
        if self.use_data_augmentation:
            ...
            self.preprocess_obs = T.Compose([
                T.Resize(self.input_size, interpolation=Image.BICUBIC),
                T.Normalize(self.clip_mean, self.clip_std)])
        # Ego4D does not include actions, we keep it to align with calvin_dataset file
        self.act_len = 10
    ...
    def __getitem__(self, index):
        ...
        if length > self.seq_len*self.video_sample_rate:
            start = np.random.choice(
                range(0, length-self.seq_len*self.video_sample_rate))
        else:
            start = 0
        ...
        padded_actions_mask = torch.zeros(
            self.seq_len, self.act_len, self.action_dim).float()  # (len, act_len, action_dim)
