...
class IP2P(nn.Module):
    ...
    def __init__(self,
        ...
        if gradient_checkpointing:
            # it will reduce GPU memory but add computing burden
    ...
    def tokenize_texts(self, texts):
        inputs = self.tokenizer(
            texts, return_tensors="pt", padding='max_length', truncation=True, max_length=77)
        return inputs
    ...
    def forward(self, input_dict):
        ...
        input_text = [text+f".And {curr_progress}% of the instruction has been finished." for (
            text, curr_progress) in zip(input_text, progress)]
        ...
        timesteps = torch.randint(
            0, self.noise_scheduler.config.num_train_timesteps, (bsz,)).to(latents.device)
        ...
        encoder_hidden_states = self.text_encoder(
            **(input_ids.to(self.device))).last_hidden_state

        # Get the additional image embedding for conditioning.
        # Instead of getting a diagonal Gaussian here, we simply take the mode.
        original_image_embeds = self.vae.encode(
            original_pixel_values).latent_dist.mode()
        ...
        if self.conditioning_dropout_prob is not None:
            ...
            null_conditioning = self.text_encoder(
                **(self.tokenize_texts([""]).to(self.device))).last_hidden_state
            ...
            prediction = self.unet(
                concatenated_noisy_latents, timesteps, encoder_hidden_states, return_dict=False)[0]
