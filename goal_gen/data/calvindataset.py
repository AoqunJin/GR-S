...
class CalvinDataset_Goalgen(Dataset):
            ...
            self.transform = transforms.Compose(
                [
                    ...
                    transforms.CenterCrop(
                        self.resolution) if self.center_crop else transforms.RandomCrop(self.resolution),
                ]
            )
        else:
    ...
    def __getitem__(self, index):
        ...
        input_image_path = os.path.join(
            self.data_dir, f"{traj_id}", f"{frame_id}_static.png")
        input_image = Image.open(input_image_path).convert("RGB")

        # goal image
        forward_n = random.choice(
            range(self.forward_n_min, self.forward_n_max + 1))
        edited_frame_id = min(frame_id + forward_n, n_frames - 1)

        assert edited_frame_id < n_frames
        edited_image_path = os.path.join(
            self.data_dir, f"{traj_id}", f"{edited_frame_id}_static.png")
        ...
        if self.is_training and self.color_aug and random.random() > 0.4:
            self.color_trans = transforms.Compose(
                [
                    transforms.ColorJitter(brightness=(bright_range, bright_range), contrast=(
                        contrast_range, contrast_range), saturation=(saturation_range, saturation_range), hue=(hue_range, hue_range)),
                ]
            )
            input_image = self.color_trans(input_image)
            # apply the same transformation to input image and goal image
            edited_image = self.color_trans(edited_image)
        # preprocess_images
