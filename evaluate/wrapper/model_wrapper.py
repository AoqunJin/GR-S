...
class CustomModel(CalvinBaseModel):
    ...
    def step(self, obs, goal, text):
        ...
        hand_rgb_tensor = torch.stack(
            self.hand_rgb_list, dim=0)  # (len, c, h, w)
        hand_rgb_data[0, :buffer_len] = hand_rgb_tensor

        # State
        arm_state, gripper_state = CustomModel.compute_rel_state(
            self.state_list)
        ...
        gripper_state_data = F.one_hot(
            gripper_state_data, num_classes=2).type_as(arm_state_data)
        ...
        # Action placeholder
        arm_action_data = torch.zeros(
            (1, self.seq_len, self.configs["policy"]['act_len'], 6))
        gripper_action_data = torch.zeros(
            1, self.seq_len, self.configs["policy"]['act_len'])
        ...
        with torch.no_grad():
            ...
            action, progress = self.policy.evaluate(
                input_dict, return_progress=True)
