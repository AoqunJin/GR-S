...
class Policy_Trainer(pl.LightningModule):
    ...
    def _initialize(self):
        ...
        mae_ckpt = 'resources/MAE/mae_pretrain_vit_base.pth'
        ...
        self.model = GR1(
            ...
            sequence_length=self.configs["policy"]['seq_len'],
        ...
        # NOTE Debug: check if all modules are used
        # def hook(module, input, output):
        #     module.has_been_used = True
        ...
        if self.configs["trainer"]["finetune"]:
            if self.configs["trainer"]["use_pretrain"]:
                ...
                self._main_rank_print(
                    f"Loading pretrained model from: {trainer_config['pretrained_model_path']}")
                checkpoint = torch.load(
                    self.configs["trainer"]['pretrained_model_path'], map_location='cpu')
                state_dict = dict()
    ...
    def configure_optimizers(self):
        ...
        self._main_rank_print(
            f"learning rate: {lr}, effective batch size: {eff_bsz}")

        optimizer = torch.optim.AdamW(
            self.model.parameters(),
            lr=lr,
            betas=(self.configs["trainer"]['betas_1'],
                   self.configs["trainer"]['betas_2']),
    ...
    def validation_step(self, batch, batch_idx):
        with torch.no_grad():
            ...
            # (b, l, act_len, act_dim - 1)
            arm_action_target = torch.clone(arm_action)
            gripper_action_target = torch.clone(
                gripper_action)  # (b, l, act_len)

            # Split arm and gripper state
            arm_state = state[:, :, :6]  # (b, l, state_dim - 1)
            gripper_state = state[:, :, 6].long()  # (b, l)
            gripper_state = F.one_hot(gripper_state, num_classes=2).type_as(
                arm_state)  # (b, l, 2)
            ...
            input_dict = {
                ...
                'state': {'arm': arm_state, 'gripper': gripper_state},
            ...
            prediction = self.model(**input_dict)

            obs_preds = prediction['obs_preds']
            obs_target = prediction['obs_targets']
            # (b, l, act_len, act_dim - 1)
            ...
            obs_hand_target = prediction['obs_hand_targets']
            # (b * l, act_latent_dim)
            # action_mu_preds = prediction['action_mu_preds']
            # (b * l, act_latent_dim)
            # action_logvar_preds = prediction['action_logvar_preds']
            ...
            if self.act_pred:
                # kl loss
                # loss_kl_act = compute_kl_divergence(
                #     action_mu_preds, action_logvar_preds)

                # action smooth l1 loss
                # b,len,act_len,6 -> b*len,act_len,6
                arm_action_preds = arm_action_preds.view(-1, act_len, act_dim-1)[
                    attention_mask.flatten() > 0]
                # b,len,act_len,6 -> b*len,act_len,6
                arm_action_target = arm_action_target.view(-1, act_len, act_dim-1)[
                    attention_mask.flatten() > 0]
                # b,len,act_len -> b*len,act_len
                action_mask = action_mask.view(-1,
                                               act_len)[attention_mask.flatten() > 0]
                # b*len*act_len, 6
                arm_action_preds = arm_action_preds.view(
                    -1, act_dim-1)[action_mask.flatten() > 0]
                # b*len*act_len, 6
                arm_action_target = arm_action_target.view(
                    -1, act_dim-1)[action_mask.flatten() > 0]
                loss_arm_act = torch.nn.SmoothL1Loss()(arm_action_preds, arm_action_target)

                # gripper bce loss
                # b,len,act_len -> b*len,act_len
                gripper_action_preds = gripper_action_preds.view(
                    -1, act_len)[attention_mask.flatten() > 0]
                # b,len,act_len -> b*len,act_len
                gripper_action_target = gripper_action_target.view(
                    -1, act_len)[attention_mask.flatten() > 0]
                gripper_action_preds = gripper_action_preds.flatten(
                )[action_mask.flatten() > 0]  # b*len*act_len
                gripper_action_target = gripper_action_target.flatten(
                )[action_mask.flatten() > 0]  # b*len*act_len
                bce_with_logits_loss = torch.nn.BCEWithLogitsLoss()
                loss_gripper_act = bce_with_logits_loss(
                    gripper_action_preds, gripper_action_target)
                # loss_act = loss_arm_act + loss_gripper_act * \
                #     self.gripper_loss_ratio + loss_kl_act * self.kl_loss_ratio
                loss_act = loss_arm_act + loss_gripper_act * \
                    self.gripper_loss_ratio

                # Compute gripper action acc
                gripper_action_preds = torch.nn.Sigmoid()(
                    gripper_action_preds)  # Sigmoid function
                gripper_action_preds = (gripper_action_preds > 0.5).float()
                acc_gripper_act = torch.eq(
                    gripper_action_preds, gripper_action_target).sum().float()
            ...
            if self.fwd_pred:
                ...
                loss_obs = (loss_obs * obs_attention_mask).sum() / \
                    obs_attention_mask.sum()
                if self.fwd_pred_hand:
                    obs_hand_preds = obs_hand_preds[:,
                                                    :seq_len-fwd_pred_next_n, :, :]
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_arm_act = 0
        loss_gripper_act = 0
        loss_kl_act = 0  # NOTE not style vector prediction
        acc_gripper_act = 0
        loss_obs = 0
        loss_hand_obs = 0
        gripper_cnt = 0
        loss_progress = 0  # NOTE not progress prediction
        # action prediction
        act_dim = self.model.act_dim
        if self.act_pred:
            # kl loss
        ...
        # NOTE Debug: check if all modules are used
        # for name, module in self.model.named_modules():
        #     if not hasattr(module, "has_been_used"):
        #         print(f"⚠️ {name} has not been used")
